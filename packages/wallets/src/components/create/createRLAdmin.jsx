...
import { AlertDialog, moonToMojo } from '@moon/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      moonper_input.value === '' ||
      Number(moonper_input.value) === 0 ||
      !Number(moonper_input.value) ||
      isNaN(Number(moonper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const moonper = moonToMojo(moonper_input.value);
    const moonper_value = Number.parseInt(Number(moonper));
    ...
    // var fee = moonToMojo(fee_input.value);
