...
import type { Row } from '@moon/core';
import { WalletType, TransactionType, toBech32m } from '@moon/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToMoon(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
