...
import { AlertDialog, Card, Flex, moonToMojo, mojoToMoonLocaleString } from '@moon/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const moonper_input = ip_parsed.limit;
    ...
    const moonper_value = Number.parseInt(Number(moonper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (moon per interval):{' '}
                  {mojoToMoonLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToMoonLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = moonToMojo(amount_input.value);
    const fee = moonToMojo(fee_input.value);
