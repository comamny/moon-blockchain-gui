...
import { SyncingStatus, toBech32m } from '@moon/api';
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send moon to coloured address. Please enter a moon address.`);
    }

    if (address.includes('moon_addr') || address.includes('colour_desc')) {
    ...
    if (address.slice(0, 12) === 'moon_addr://') {
