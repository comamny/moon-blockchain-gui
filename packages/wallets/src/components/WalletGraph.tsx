...
import { TransactionType } from '@moon/api';
import type { Transaction } from '@moon/api';
import { mojoToMoon, blockHeightToTimestamp } from '@moon/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToMoon(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToMoon(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToMoon(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToMoon(start).toString, // bignumber is not supported by react
    });
  });
