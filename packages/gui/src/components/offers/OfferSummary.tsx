...
import { type OfferSummaryRecord } from '@moon/api';
import {
  Flex,
  FormatLargeNumber,
  StateColor,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['moc', 'tmoc'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToMoon(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['moc', 'tmoc'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToMoon(takerEntries[0][1])
