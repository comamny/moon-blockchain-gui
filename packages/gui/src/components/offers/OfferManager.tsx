...
import { OfferSummaryRecord, OfferTradeRecord } from '@moon/api';
import {
  Box,
  Checkbox,
  Chip,
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: mocFee } = methods.getValues();

    const fee = cancelWithTransaction ? moonToMojo(mocFee) : new BigNumber(0);
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'TMOC';
        ...
        <Grid container>
          <Grid xs={12} md={6} lg={5} item>
            <CardHero>
              ...
              <Typography variant="body1">
                <Trans>
                  Create an offer to exchange assets including MOC, tokens, and
