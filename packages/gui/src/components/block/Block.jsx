...
import { useGetBlockQuery, useGetBlockRecordQuery } from '@moon/api-react';
...
export default function Block() {
  ...
  const poolReward = mojoToMoon(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToMoon(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const moonFees =
    blockRecord.fees !== undefined ? mojoToMoon(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: moonFees ? `${moonFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Moon blockchain
            </Trans>
          </Back>
        }
        action={
