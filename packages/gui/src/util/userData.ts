...
export function getUserDataDir(): string {
  const moonRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(moonRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const moonRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${moonRootUserDataPath}`);
  app.setPath('userData', moonRootUserDataPath);
}
