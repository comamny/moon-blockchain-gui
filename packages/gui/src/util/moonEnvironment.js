...
const getMoonVersion = () => {
  let version = null;
  const exePath = getExecutablePath('moon');
  // first see if we can get a moon exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or moon is on the path
    try {
      version = child_process
